package com.github.cloud0072.base.schedule;
...
//    @Scheduled(cron = "0 * * * * ?")
    public void test() {
        // DateTimeFormatter为线程安全格式化工具
        DateTimeFormatter df = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.SSS");
        //LocalDateTime jdk8新版时间类
        LocalDateTime ldt = LocalDateTime.now();
        log.info("当前时间:\t{}", ldt.format(df));
    }

//　　（1）*：表示匹配该域的任意值。假如在Minutes域使用*, 即表示每分钟都会触发事件。
//
//　　（2）?：只能用在DayofMonth和DayofWeek两个域。它也匹配域的任意值，但实际不会。
//          因为DayofMonth和DayofWeek会相互影响。例如想在每月的20日触发调度，
...
//          意味着在最后的一个星期四触发。
//
//　　（7）W:表示有效工作日(周一到周五),只能出现在DayofMonth域，
//          系统将在离指定日期的最近的有效工作日触发事件。例如：
//          在 DayofMonth使用5W，如果5日是星期六，则将在最近的工作日：星期五，即4日触发。
