...
$.extend(DayGrid.prototype, {
	...
	getCellDate: function(cell) {
		return this.view.cellToDate(cell); // leverages the View's cell common
	},
	...
	// Converts a range with an inclusive `start` and an exclusive `end` into an array of segment objects
	rangeToSegs: function(start, end) {
		return this.view.rangeToSegments(start, end); // leverages the View's cell common
	},
...
$.extend(DayGrid.prototype, {
	...
	renderSegPopoverContent: function(date, segs) {
		...
		for (i = 0; i < segs.length; i++) {
			...
			// because segments in the popover are not part of a grid coordinate common, provide a hint to any
...
$.extend(TimeGrid.prototype, {
	...
	rangeToSegs: function(rangeStart, rangeEnd) {
		...
		for (col = 0; col < view.colCnt; col++) {
			cellDate = view.cellToDate(0, col); // use the View's cell common for this
	...
	getCellDate: function(cell) {
		...
		return calendar.rezoneDate( // since we are adding a time, it needs to be in the calendar's timezone
			view.cellToDate(0, cell.col) // View's coord common only accounts for start-of-day for column
